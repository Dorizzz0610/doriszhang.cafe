...
async function getPostData() {
  ...
  // 查找理论内容结束位置，添加结束标签
  const sections = contentHtml.split(/<div class="theory-block/);
  for (let i = 1; i < sections.length; i++) {
    // 查找下一个理论块或分隔线或H2标题开始的位置
    const endIndex = sections[i].search(/<div class="theory-block|<div class="flex items-center my-12"|<hr>|<h2/);
    if (endIndex === -1) {
      // 如果没有找到下一个区块，则关闭当前区块
      sections[i] += '</div></div>';
    } else {
        ...
        <main className="container mx-auto px-4 sm:px-6">
          ...
          >
            ...
            <article className="journal-article backdrop-blur-sm bg-white/30 dark:bg-gray-800/10 rounded-2xl p-8 mb-12">
              {title && (
                <div className="text-sm font-semibold text-gray-800/90 dark:text-gray-200/90 mb-2">
                  {title}
                </div>
              )}
              {date && (
